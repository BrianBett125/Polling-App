...
CREATE TABLE IF NOT EXISTS public.votes (
  ...
  ip_address TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  -- Constraint to ensure one vote per user per poll
  CONSTRAINT unique_user_poll_vote UNIQUE (poll_id, user_id) NULLS NOT DISTINCT,
  -- Constraint to ensure one vote per IP address per poll (for anonymous users)
...
-- Create function to vote for an option
...
BEGIN
  ...
  -- Check if the user or IP has already voted on this poll
  ...
  VALUES (p_poll_id, p_option_id, v_user_id, CASE WHEN v_user_id IS NULL THEN p_ip_address ELSE NULL END)
  RETURNING id INTO v_vote_id;
  
  -- Increment the vote count
  PERFORM increment_vote(p_option_id);
